...
```{r}
HappyxSad<- 
    get_playlist_audio_features('11122548577','1C15xY2yKcbmOU7xzNwFlJ') %>% 
    add_audio_analysis %>% 
    mutate(
        segments = 
            map2(segments, key, compmus_c_transpose)) %>% 
...
HappyxSad_juice <- 
    ...
    step_scale(all_predictors()) %>%
    # step_range(all_predictors()) %>% 
    prep(HappyxSad %>% mutate(track_name = str_trunc(track_name, 20))) %>% 
    juice %>% 
    column_to_rownames('track_name')
```

```{r}
HappyxSad_dist <- dist(HappyxSad_juice, method = 'euclidean')
```

```{r}
hclust(HappyxSad_dist, method = 'single') %>% dendro_data %>% ggdendrogram
```

```{r}
protoclust(HappyxSad_dist) %>% dendro_data %>% ggdendrogram
```

```{r}
kmeans(HappyxSad_juice, 2)
```
