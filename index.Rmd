...
![comparementgraph](ComparementGraph.png)
...
###'Poppy' versus instrumental  Chroma
    ...
    mutate(
        timbre = 
            map(segments, 
                ...
                method = 'mean'))
Chocolate <- 
    get_tidy_audio_analysis('4WiiRw2PHMNQE0ad6y6GdD') %>% 
    compmus_align(bars, segments) %>% 
    select(bars) %>% unnest(bars) %>% 
    mutate(
        pitches = 
...
Robbersplot <- 
    bind_rows(
        Robbers %>% compmus_self_similarity(pitches, 'aitchison') %>% mutate(d = d / max(d), type = "Chroma"),
      Robbers %>% compmus_self_similarity(timbre, 'euclidean') %>% mutate(d = d / max(d), type = "Timbre")) %>% 
    ggplot(
        aes(
            x = xstart + xduration / 2, 
            width = xduration,
    ...
    theme_classic() +
    labs(x = '', y = '')
ggplotly(Robbersplot)

Chocolateplot <- 
    bind_rows(
        Chocolate %>% compmus_self_similarity(pitches, 'aitchison') %>% mutate(d = d / max(d), type = "Chroma"),
       Chocolate %>% compmus_self_similarity(timbre, 'euclidean') %>% mutate(d = d / max(d), type = "Timbre")) %>% 
    ggplot(
        aes(
            x = xstart + xduration / 2, 
            width = xduration,
    ...
    theme_classic() +
    labs(x = '', y = '')
ggplotly(Chocolateplot)

PleaseBeNakedplot <- 
    bind_rows(
        PleaseBeNaked %>% compmus_self_similarity(pitches, 'aitchison') %>% mutate(d = d / max(d), type = "Chroma"),
        PleaseBeNaked %>% compmus_self_similarity(timbre, 'euclidean') %>% mutate(d = d / max(d), type = "Timbre")) %>% 
    ggplot(
        aes(
            x = xstart + xduration / 2, 
            width = xduration,
